...
import com.ltcn272.finny.presentation.features.transation.transaction_list.component.RecurringTransactionLink
import com.ltcn272.finny.presentation.features.transation.transaction_list.component.TransactionDayGroup
import com.ltcn272.finny.presentation.features.transation.transaction_list.component.TransactionDayGroupShimmer
...
fun TransactionScreen(
    ...
    val isInitialTransactionLoading = lazyTransactions.loadState.refresh is LoadState.Loading && lazyTransactions.itemCount == 0
    val isInitialBudgetLoading = lazyBudgets.loadState.refresh is LoadState.Loading && lazyBudgets.itemCount == 0
    ...
    val groupedTransactions = remember(lazyTransactions.itemSnapshotList) {
        lazyTransactions.itemSnapshotList.items
            ...
            .map { (date, dailyTransactions) ->
                val total = dailyTransactions.sumOf { if (it.type == TransactionType.INCOME) it.amount else -it.amount }
                DaySection(
                    date = date,
                    totalAmount = total,
    ...
    FinnyTheme {
        Column(
            ...
            if (isInitialBudgetLoading) {
                BudgetFilterRowShimmer(modifier = Modifier.padding(start = 16.dp, end = 16.dp, top = 4.dp))
            } else {
                BudgetFilterRow(
